...
def real_sph_harm(l, zero_m_only=False, spherical_coordinates=True):
    ...
    for i in range(l):
        Y_func_l_m[i][0] = sym.simplify(
            sph_harm_prefactor(i, 0) * P_l_m[i][0])
    if not zero_m_only:
        for i in range(1, l):
            for j in range(1, i + 1):
                Y_func_l_m[i][j] = sym.simplify(
                    2**0.5 * sph_harm_prefactor(i, j) *
                    C_m[j] * P_l_m[i][j])
        for i in range(1, l):
            for j in range(1, i + 1):
                Y_func_l_m[i][-j] = sym.simplify(
...
class dist_emb(paddle.nn.Layer):
    def __init__(self, num_radial, cutoff=5.0, envelope_exponent=5):
        ...
        self.freq = paddle.create_parameter(
            ...
            default_initializer=paddle.nn.initializer.Assign(
                paddle.arange(1, num_radial + 1, dtype='float32') * PI
            ),
        )
...
class angle_emb(paddle.nn.Layer):
    def __init__(self, num_spherical, num_radial, cutoff=5.0,
        ...
        for i in range(num_spherical):
            if i == 0:
                sph1 = sym.lambdify([theta], sph_harm_forms[i][0],
                                    modules)(0)
                self.sph_funcs.append(
                    partial(self._sph_to_tensor, sph1))
            else:
    ...
    def forward(self, dist, angle, idx_kj):
        ...
        rbf = paddle.stack(
            x=[f(dist) for f in self.bessel_funcs], axis=1)
        cbf = paddle.stack(
            x=[f(angle) for f in self.sph_funcs], axis=1)
        n, k = self.num_spherical, self.num_radial
        out = (rbf[idx_kj].reshape([-1, n, k]) *
               cbf.reshape([-1, n, 1])).reshape([-1, n * k])
        return out


class torsion_emb(paddle.nn.Layer):
    def __init__(self, num_spherical, num_radial, cutoff=5.0,
                 envelope_exponent=5):
        ...
        for i in range(self.num_spherical):
            if i == 0:
                sph1 = sym.lambdify([theta, phi], sph_harm_forms[i][0],
                                    modules)
                self.sph_funcs.append(
                    partial(self._sph_to_tensor_2d, sph1(0, 0)))
            else:
                for k in range(-i, i + 1):
                    sph = sym.lambdify([theta, phi],
                                       sph_harm_forms[i][k + i], modules)
                    self.sph_funcs.append(sph)
    ...
    @staticmethod
    def _sph_to_tensor_2d(sph_val, x, y):
        return paddle.zeros_like(x=x) + paddle.zeros_like(x=y) + sph_val
    ...
    def forward(self, dist, angle, phi, idx_kj):
        ...
        rbf = paddle.stack(
            x=[f(dist) for f in self.bessel_funcs], axis=1)
        cbf = paddle.stack(
            x=[f(angle, phi) for f in self.sph_funcs], axis=1)
        n, k = self.num_spherical, self.num_radial
        out = (rbf[idx_kj].reshape([-1, 1, n, k]) *
               cbf.reshape([-1, n, n, 1])).reshape([-1, n * n * k])
        return out
